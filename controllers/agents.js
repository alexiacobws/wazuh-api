...
/**
 * @api {get} /agents/unassigned Get agents not assigned to any group
 * @apiName GetAgentUnassigned
 * @apiGroup Groups
 *
 * @apiParam {Number} [offset] First element to return in the collection.
 * @apiParam {Number} [limit=500] Maximum number of elements to return.
 ...
 *
 * @apiExample {curl} Example usage:
 *     curl -u foo:bar -k -X GET "https://127.0.0.1:55000/agents/unassigned?pretty"
 *
 */
router.get('/unassigned', cache(), function(req, res) {
    logger.debug(req.connection.remoteAddress + " GET /agents/unassigned");

    req.apicacheGroup = "agents";

    var data_request = {'function': '/agents/unassigned', 'arguments': {}};
    var filters = {'offset': 'numbers', 'limit': 'numbers', 'sort':'sort_param', 'search':'search_param', 'select':'select_param'};

    data_request['arguments']['group_id'] = "unassigned";
